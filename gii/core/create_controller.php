...
class " . $c . " extends CI_Controller
{
    ...
    \n\n 
    /* ---------------------------------------------------------------- 
    Nama \t: Upload ".ucfirst($table_name)." 
    Fungsi \t: Mengunggah file ke direktori dan disimpan di tabel ".ucfirst($c_url)."
    Method \t: POST   
    ---------------------------------------------------------------- */
    public function upload() 
    {
     \$idBaru = \$_POST['id_baru'];
     \$target_path_folder = './images/".$c_url."/big/';
     echo \$fileName = \$idBaru.time().basename(\$_FILES['club_image']['name']);
     \$imageName = \$this->".$m."->getImageName(\$idBaru);

     if(\$imageName != ''){
         \$data = array(
         'image' => \$imageName.','.\$fileName,
         );
         \$this->".$m."->update(array('id' => \$idBaru), \$data);
